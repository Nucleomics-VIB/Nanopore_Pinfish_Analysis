...
Best quality read data was produced by re-basecalling the raw ONT data with Guppy (version 3.0.6) in high accuracy calling mode (hac).
...
The table below reports the counts of the different entities found with the yeast dataset.
...
# pycIds <- gsub(".+ ", "", as.character(ShortRead::id(readFastq(pycData))))
...
Based on the Full BAM data and the genome coordinate of all exons (current reference genome: **'R64-1-1.97'**), we computed how many of the reference exons had coverage and counted the corresponding genes (keeping the average exon coverage as final coverage value). In this section, we made use of the tool **'mosdepth'** (https://github.com/brentp/mosdepth) and some R magic.
...
The next table reports the top`r topexp` most expressed genes
...
ids <- tail(cvg.dat, topexp)[,1]$gene
```

# Functional enrichment results from the Top20 expressed genes

The biological functions carried by the top 20 expressed genes were identified using '''YeastMine''' (https://yeastmine.yeastgenome.org) which is part of the '''InterMine''' suite [@InterMine].
...
# substituted by the correct database
...
#ids <- c("YBR072W", "YBR118W", "YHR174W", "YKL060C", "YKL152C", "YKL153W", "YDR077W", "YGL055W", "YGL102C", "YGL103W", "YGR192C", #"YGR254W", "YJL052W", "YJL159W", "YLR044C", "YLR110C", "YMR251W-A", "YNL031C", "YOL052C-A", "YPR036W-A")
entrez <- xx[ids]
...
```{r MinerTop, echo=FALSE, cache=FALSE, resize.width=100}
knitr::kable(PD_FDR_enrichResult$data[1:10,], 
...
```{r MinerTop2, echo=FALSE, cache=FALSE, resize.width=100}
...
The table above shows the transcripts that have been assigned to the class **j: multi-exon with at least one junction match**. This classification could include a variety of novel isoforms - these can be checked using a genome browser. An example of using IGV is covered in the next section
...
We recommend using **`IGV`** [@IGV] to display the raw and processed data in teh context of the public reference. The command below will open the **`IGV`** browser to the display the main data from this analysis.
...
```{r, echo=FALSE}
knitr::include_graphics("Static/Images/graph.pdf")
...
${condapath}/conda list -n Pinfish "samtools|minimap2|snakemake|rstudio|pinfish|pychopper|racon|gffread|gffcompare|igv|openjdk|mosdepth"
```
